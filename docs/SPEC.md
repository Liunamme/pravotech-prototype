...
Если спека чего-то не покрывает — **не выдумывать**, а поднять вопрос до принятия решения.
...
- `npm run lint`, `npm test`, `npm run dry-run` — зелёные.
- `npm run check:dist` — собранный CSS не потерял стандартные свойства (см. ловушку 13).

---

## 6. Ловушки платформы

Каждая — из тех, на которые уже наступали. Комментарии в коде ссылаются
сюда по номерам.

1. **`backdrop-filter` ломает прокрутку.** Элемент с фильтром становится
   «backdrop root», и композитор Chromium перестаёт считать его целью колеса
   — событие уходит тому, что лежит ПОД ним. Лечение: фильтр выносится в
   отдельный инертный слой (`pointer-events: none`), содержимое лежит выше и
   фильтра не имеет. Так сделано в `DocumentInspector` (`.glass`),
   `QueueDrawer`, `MobileThreadsDrawer`, `MobileSettingsSheet`. Если где-то
   ещё «проваливается» ввод — это та же болезнь.

2. **`mask-image` на предке отключает `backdrop-filter` внутри.** Предок с
   маской — тоже backdrop root. Поэтому затухание краёв делается маской
   только там, где внутри нет стекла (`DeadlineTimeline`, вкладки контекста),
   а где есть — вуалью-соседом (край приглашения в `WorkspacePage`).
   В `CardQueue` маска допустима осознанно: за карточками ровная подложка,
   размывать там нечего.

3. **Родной `scrollIntoView` тащит всю раскладку.** Он подкручивает КАЖДОГО
   предка, включая `overflow: hidden` и саму страницу. Использовать только
   `shared/lib/scrollIntoViewWithin.ts` — он двигает единственный контейнер.

4. **`Tooltip.Trigger asChild` + `NavLink className={fn}`.** Radix склеивает
   className строкой, и функция попадает в атрибут своим исходным текстом —
   стили не применяются вовсе. Активное состояние ссылок брать из
   `[aria-current='page']`.

5. **Акцентный цвет на акцентной подложке сливается.** На фиолетовом фоне
   текст берёт `--pt-text` (белый в тёмной теме, тёмный в светлой). Тот же
   принцип у активной иконки рейки и активной вкладки нижней навигации.

6. **Скрим модалки не должен брать `--color-text`** — в тёмной теме он почти
   белый. Для этого есть отдельный токен `--pt-scrim`.

7. **Родные контролы** (иконка `input[type=date]`, календарь, выпадающие
   списки) красит только `color-scheme` — он задан в `tokens.css` для обеих тем.

8. **Никаких literal-цветов вне `tokens.css`.** Проверка:
   `grep -rnE '#[0-9a-fA-F]{3,6}' src --include='*.module.css'`.

9. **Полосу под логотип резервирует `--logo-safe-top`** (рейка, настройки,
   витрина) — иначе логотип ложится на заголовок страницы.

10. **React 19 считает возвращённое ref-колбэком значение функцией очистки.**
    `ref={(el) => el && f(el)}` не проходит типизацию: тело колбэка должно
    быть в скобках и ничего не возвращать.
...
13. **Вендорный префикс пишется ПЕРЕД стандартным свойством.** Минификатор
    ...
    из сборки выживал только `-webkit-backdrop-filter`, и матовое стекло
...
14. **Два места, обязанных выглядеть одинаково, должны быть одним
    компонентом.** Копия чипов подсказок на экране приглашения дважды
    разъезжалась с оригиналом из пустого треда, пока копию не удалили.

15. **Переполнение — не прокручиваемость.** Содержимое выше экрана ещё не
    значит, что список прокручивается: без `overflow` он просто обрезается.
    Проверять сменой `scrollTop`, а не сравнением высот.
